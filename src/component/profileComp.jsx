...
class ProfileComp extends React.Component {
  ...
  render() {
    ...
    function ProfileCompContent({ showMenu, isMobile,lightMode }) {
      return (
        <>
          <div
            className={`profile columns-auto mx-auto container flex items-center h-fit `}
          >
            <div className={`font-Poppins w-wrapper mx-auto flex flex-col items-center ${lightMode?'text-DarkBlueText':''}`}>
              <div className={`${isMobile ? "mx-[20px]" : "about"}`}>
                <h1 className={`text-center text-3xl ${lightMode?'text-DarkBlueText':'text-LightPink'}`}>ABOUT ME</h1>
                <div className="part1">
    ...
    return (
      <>
        <ProfileCompContent lightMode={lightMode}
        isMobile={isMobile}
        showMenu={showMenu}/>
      </>
    );
